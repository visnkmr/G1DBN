##check for "Change name here" and enter the desired name
library(G1DBN)
p <- 10
n <- 20
geneProp <- 0.05
...
B <- runif(p,-1,1)
sigmaEps <- runif(p,0.1,0.5) ## generates 10 random numbers between 0.1 and 0.5
X0 <- B + rnorm(p,0,sigmaEps*10) # sample a normal distribution, with a mean of 0 and sd of sigmaEps*10, 10 times
...
plot(G1toPlot, layout=nodeCoord, vertex.label = c("Change name here","B","C","D","E","F","G","H","I","J"), edge.arrow.size = 0.2, main="G1DBN Inferred network:\n Step 1")
screen(2)
...
plot(G2toPlot, layout=nodeCoord, vertex.label = c("Change name here","B","C","D","E","F","G","H","I","J"),edge.arrow.size = 0.2, main="G1DBN Inferred network:\n Step 2")
...
plot(G0toPlot, layout=nodeCoord, vertex.label = c("Change name here","B","C","D","E","F","G","H","I","J"), edge.arrow.size = 0.2, main="Simulated network:")
